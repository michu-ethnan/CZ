...
if (ALERT_BOX.resolveFor(actor).isPresent()){
    ...
    actor.attemptsTo(
            Check.whether(ALERT_BOX.resolveFor(actor).containsText("Produkt byl přidán do košíku")).andIfSo(MoveMouse.to(ALERT_BOX)).otherwise(ReturnAndAddAnotherProduct.toCart())
    );
}
else {
    actor.attemptsTo(
